...
class CryptoEnv(gym.Env):
    ...
    def step(self, action: int):
        ...
        if self.position != 0:
            reward = (c - self.entry_price) / (self.entry_price + 1e-10)
            self.position = 0
            self.entry_price = 0.0
        ...
        truncated = False
        obs = self._get_obs()
        info = {"price": float(c), "position": self.position, "rsi": None, "bb_pos": None}
        ...
        try:
            if action_idx == 1:
                ...
                if position < 0:
                    ...
                    time.sleep(0.5)
                # Open/continue long
                alloc_balance = balance * 0.25  # 25% per ticker max
                qty = alloc_balance / price
                ...
                # HOLD but we have position — optionally take profit/stop
                pass  # PPO said HOLD, keep position
        except Exception as e:
